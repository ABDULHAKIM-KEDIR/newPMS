            ...
            <a href="{{ route('tasks.index') }}" class="nav-item {{ request()->routeIs('tasks.*') ? 'active' : '' }}">
                ...
                @if ($myTasksCount > 0)
                    <span class="nav-badge"
                        style="background:var(--accent-soft); color:var(--accent-dark);">{{ $myTasksCount }}</span>
                @endif
            </a>
        @endcan
        ...
        @if ($currentUser->can('manage_budgets') || $currentUser->isDirectorOrAdmin())
            <a href="{{ route('budgets.index') }}" class="nav-item {{ request()->routeIs('budgets.*') ? 'active' : '' }}">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <path
                        d="M12 7v10M9.5 9.2c0-1.2 1.1-2 2.5-2s2.5.9 2.5 2c0 3-5 1.6-5 4.6 0 1.2 1.1 2 2.5 2s2.5-.8 2.5-2" />
                </svg>
                <span>Budgets</span>
            </a>
        @endif
