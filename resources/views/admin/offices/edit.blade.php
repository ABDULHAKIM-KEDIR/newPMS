    ...
    <form method="POST" action="{{ route('admin.offices.update', $office) }}">
      ...
      <div class="form-field">
        ...
        <select id="unit_type" name="unit_type">
          <option value="Office" {{ old('unit_type', $office->unit_type ?? 'Office') === 'Office' ? 'selected' : '' }}>Office</option>
          <option value="Organization" {{ old('unit_type', $office->unit_type) === 'Organization' ? 'selected' : '' }}>Organization</option>
          <option value="Directorate" {{ old('unit_type', $office->unit_type) === 'Directorate' ? 'selected' : '' }}>Directorate</option>
          <option value="Department" {{ old('unit_type', $office->unit_type) === 'Department' ? 'selected' : '' }}>Department</option>
          <option value="Sub-department" {{ old('unit_type', $office->unit_type) === 'Sub-department' ? 'selected' : '' }}>Sub-department</option>
          <option value="Branch Office" {{ old('unit_type', $office->unit_type) === 'Branch Office' ? 'selected' : '' }}>Branch Office</option>
          <option value="Unit" {{ old('unit_type', $office->unit_type) === 'Unit' ? 'selected' : '' }}>Unit / Team</option>
        </select>
      </div>
      <div class="form-field">
        ...
        <select id="parent_office_id" name="parent_office_id">
          ...
          @foreach ($parentOffices as $po)
            <option value="{{ $po->office_id }}" {{ (string) old('parent_office_id', $office->parent_office_id) === (string) $po->office_id ? 'selected' : '' }}>
